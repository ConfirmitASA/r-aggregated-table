...
describe('TableData', () => {
  ...
  it('.getData should get data',()=>{
    ...
    let multidimensional = TableData.detectMultidimensional(TNRH);
  ...
  it('.detectMultidimensional should detect multidimensional setup without passed column',()=>{
    expect(TableData.detectMultidimensional($j('#table-nested-rowheaders')[0])).toBe(true);
    expect(TableData.detectMultidimensional($j('#table-nested-headers')[0])).toBe(false);
  });
});
